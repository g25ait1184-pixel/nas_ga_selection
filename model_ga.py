...
class Architecture:
    ...
    def __repr__(self):
        return f"Arch(conv={self.genes['num_conv']}, acc={self.accuracy:.4f}, fit={self.fitness:.4f})"
...
class GeneticAlgorithm:
    ...
    def evaluate_fitness(self, architecture, train_loader, val_loader, device, epochs=100,
        ...
        try:
            ...
            # Count parameters and split conv vs fc
            conv_params = 0
            fc_params = 0
            ...
            # fallback total
            num_params = conv_params + fc_params
            if num_params == 0:
                num_params = sum(p.numel() for p in model.parameters())
                # naive split if we couldn't infer layer names
            ...
            # Weighted penalty (conv vs fc)
            penalty_weighted = w_conv * conv_m + w_fc * fc_m
            fitness_weighted = best_acc - penalty_weighted

            # Save into architecture (both values)
            ...
            architecture.fitness_original = 0
            architecture.fitness_weighted = 0
            architecture.accuracy = 0
            return 0
    
    def tournament_selection(self):
        """Tournament selection (existing behavior)"""
        tournament_size = 3
        selected = []
    ...
    def selection(self, method='tournament'):
        ...
        if method == 'tournament':
            ...
            raise ValueError("Unknown selection method: choose 'tournament' or 'roulette'")
    ...
    def evolve(self, train_loader, val_loader, device, run=1, selection_method='tournament'):
        ...
        for generation in range(self.generations):
            ...
            # Sort by fitness score (the canonical fitness field is kept as original for backward compatibility)
            ...
            # Selection
            print(f"\nPerforming selection (method={selection_method}) of total population: {self.population_size} ...", flush=True)
            selected = self.selection(method=selection_method)
            ...
            os.makedirs(os.path.join(parent, 'outputs', f'run_{run}'), exist_ok=True)
